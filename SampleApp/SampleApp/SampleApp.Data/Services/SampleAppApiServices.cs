...
using SampleApp.Common.Exceptions;
...
{
	public class SampleAppApiServices : ISampleAppApiServices
	{
		...
		public SampleAppApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new SampleAppApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw SampleAppApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new SampleAppApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
