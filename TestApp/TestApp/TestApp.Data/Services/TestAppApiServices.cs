...
{
	public class TestAppApiServices : ITestAppApiServices
	{
		...
		public TestAppApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new TestAppApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw TestAppApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new TestAppApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
