...
using FastBar.Common.Exceptions;
using FastBar.Common.Interfaces;
using FastBar.Common.Logging;
using FastBar.Data.Interfaces;

namespace FastBar.Data.Services
{
	public class FastBarApiServices : IFastBarApiServices
	{
		...
		public FastBarApiServices(IAppConfig appConfig, ILogger logger, IConnectivityHelper connectivityHelper)
		{
		...
		private async Task<T> ExecuteRequest<T>(string api, object data, Uri url, HttpClient client, HttpRequestMessage request)
		{
			if (!_connectivityHelper.IsConnected)
				throw new FastBarApiExceptions(ApiErrorCodes.ConnectivityLost, "No connectivity found when about to make an api request");
			...
			{
				throw FastBarApiExceptions.ProcessApiException(ex, id, api, request.Method.ToString(), url.ToString(), data);
			}
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new FastBarApiExceptions(ApiErrorCodes.GenericError, response.ReasonPhrase) { ResponseContent = responseContent };
			}
